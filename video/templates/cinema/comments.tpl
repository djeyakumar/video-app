{if $allow_comments == '1'}
	...
	{if $comment_system_facebook}
		<div class="tab-pane fade {if $comment_system_primary == 'facebook'}in active{/if} pm-comments comment_box" id="comments-facebook">
			{literal}
			<div class="fb-comments" data-href="{/literal}{if $tpl_name == 'article-read'}{$article.link}{else}{$video_data.video_href}{/if}{literal}" data-numposts="{/literal}{$fb_comment_numposts}{literal}" data-order-by="{/literal}{$fb_comment_sorting}{literal}" data-colorscheme="light" data-width="758"></div>
			{/literal}
		</div>
	{/if}

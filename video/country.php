	...
	case 'month':
		...
		$sql = "SELECT COUNT(*) as total_found 
				...
				WHERE added >= '". $time_start ."' 
				  AND added <= '". $time_end ."' AND country_id = '".$country_id."'";
		$result = @mysql_query($sql);
...
if ($total_videos > 0)
{
	switch ($date)
	{
		default: 
			...
			$sql = "SELECT id  
					FROM pm_videos 
					WHERE added <= '". $time_now_minute ."' AND country_id = '".$country_id."' 
					ORDER BY added DESC 
					LIMIT ". $from .",". $limit;
		break;
...
{
	$filename = (_SEOMOD) ? 'country.html' : 'country.php';
...
$smarty->assign('page_title', $country_name);

	...
	case 'month':
		...
		$sql = "SELECT COUNT(*) as total_found 
				...
				WHERE added >= '". $time_start ."' 
				  AND added <= '". $time_end ."' AND video_type = '".$video_type."'";
		$result = @mysql_query($sql);
...
if ($total_videos > 0)
{
	switch ($date)
	{
		default: 
			...
			$sql = "SELECT id  
					FROM pm_videos 
					WHERE added <= '". $time_now_minute ."' AND video_type = '".$video_type."' 
					ORDER BY added DESC 
					LIMIT ". $from .",". $limit;
		break;
...
{
	$filename = (_SEOMOD) ? 'videotype.html' : 'videotype.php';
...
// end
$page_title = $_GET['type'] == 0 ? "Movies" : "TV-Series";
